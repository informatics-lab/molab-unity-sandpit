Shader "Custom/HeightmapTerrain" {
	...
	SubShader {
		...
		{
			// _Heightmap = black and red image?
			// _HeightMatrix = size of matrix // scale colour value to height value
			// Position of vertex on texture
			float4 pos = float4( TRANSFORM_TEX( v.vertex.xz, _Heightmap ), 1, 1 );
			//height is the texture at that coordinate
			// value ranges from 0-1
			float4 height = tex2Dlod(_Heightmap,pos);

			//-------------- this is all for the normal information -----------
			// find location of next vertex position (and to the right)
			// forward and right are the unit vectors in texture space
			...
			// define the difference in colour(proxy for height) between the current position + 1 step and - 1 step 
			...
			// create the normal for each pixel
